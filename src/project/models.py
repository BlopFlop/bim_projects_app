...
class Project(Base):
    ...
    name: Mapped[str] = mapped_column(
        String(150),
        CheckConstraint(
            "0 < LENGTH(name) <= 150", name="check_len_name_project"
        ),
        unique=True,
        nullable=False,
        comment=(
    ...
    description: Mapped[str] = mapped_column(
        String(350),
        CheckConstraint(
            "0 < LENGTH(description) <= 350", name="check_len_desc_project"
        ),
        nullable=False,
        comment=(
    ...
    code: Mapped[str] = mapped_column(
        String(50),
        CheckConstraint("0 < LENGTH(code) <= 50", name="check_len_code"),
        nullable=False,
        unique=True,
        comment=(
    ...
    image: Mapped[str] = mapped_column(
        String(150),
        CheckConstraint("0 <= LENGTH(image) <= 150", name="check_len_image"),
        nullable=True,
        comment=(
    ...
    base_path: Mapped[str] = mapped_column(
        String(200),
        CheckConstraint(
            "1 < LENGTH(base_path) <= 200", name="check_len_base_path"
        ),
        nullable=True,
        comment=(
    ...
    arch_path: Mapped[str] = mapped_column(
        String(200),
        CheckConstraint(
            "0 < LENGTH(arch_path) <= 200", name="check_len_arch_path"
        ),
        nullable=True,
        comment=(
    ...
    ftp_path: Mapped[str] = mapped_column(
        String(200),
        CheckConstraint(
            "0 < LENGTH(ftp_path) <= 200", name="check_len_ftp_path"
        ),
        nullable=True,
        comment=(
    ...
    models: Mapped[list["BIMModel"]] = relationship(
        "BIMModel", back_populates="project", cascade="all, delete-orphan"
    )
...
class RevitServer(Base):
    ...
    name: Mapped[str] = mapped_column(
        String(50),
        CheckConstraint(
            "0 < LENGTH(name) <= 50", name="check_len_name_revitserver"
        ),
        unique=True,
        nullable=False,
        comment=(
    ...
    models: Mapped[list["BIMModel"]] = relationship(
        "BIMModel", back_populates="server", cascade="all, delete-orphan"
    )
...
class ModelSection(Base):
    ...
    name: Mapped[str] = mapped_column(
        String(150),
        CheckConstraint(
            "0 < LENGTH(name) <= 150", name="check_len_name_section"
        ),
        unique=True,
        nullable=False,
        comment=(
    ...
    description: Mapped[str] = mapped_column(
        String(350),
        CheckConstraint(
            "0 < LENGTH(description) <= 350", name="check_len_desc_section"
        ),
        nullable=False,
        comment=(
    ...
    # relation fields
    models: Mapped[list["BIMModel"]] = relationship(
        "BIMModel", back_populates="section", cascade="save-update"
    )
...
class BIMModel(Base):
    ...
    name_file: Mapped[str] = mapped_column(
        String(150),
        CheckConstraint(
            "0 < LENGTH(name_file) <= 150", name="check_len_name_file"
        ),
        nullable=False,
        comment=(
    ...
    type: Mapped[ModelTypeEnum] = mapped_column(
        default=ModelTypeEnum.SUPPORT, comment="Тип модели."
    )
    version: Mapped[int] = mapped_column(
        Integer,
        CheckConstraint("2000 <= version <= 2100", name="check_version_file"),
        nullable=False,
        comment=(
    ...
    extention: Mapped[str] = mapped_column(
        String(10),
        CheckConstraint(
            "1 < LENGTH(extention) <= 10", name="check_len_extention"
        ),
        unique=False,
        nullable=False,
        comment=(
    ...
    path: Mapped[str] = mapped_column(
        String(200),
        CheckConstraint("1 < LENGTH(path) <= 200", name="check_len_path"),
        nullable=False,
        comment=(
            "Путь до файла, обязательное строковое поле;"
            " допустимая длина строки — от 1 до 200 символов включительно;"
    ...
    section: Mapped[ModelSection] = relationship(
        "ModelSection", back_populates="models"
    )
    ...
    server: Mapped[RevitServer] = relationship(
        "RevitServer", back_populates="models"
    )
